...
! function(a) {
    ...
    b.prototype = {
        ...
        "mousedown": function(b) {
            if(b.target.classList.contains('slider-selection') || b.target.classList.contains('slider-handle')){
                $initIndex = b.target.parentElement.previousElementSibling.getAttribute('data-index');
                $initKms = b.target.parentElement.previousElementSibling.getAttribute('data-kms');
            }if(b.target.classList.contains('slider-track')){
                $initIndex = b.target.previousElementSibling.getAttribute('data-index');
                $initKms = b.target.previousElementSibling.getAttribute('data-kms');
            }

            if($slideType == 1) {setTimeout(()=>{$('#PorcentajesKilometraje label.perc[data-index=\"'+ $initIndex+'\"]').text(100-c+'%');$('#PorcentajesKilometraje label.kms[data-index=\"'+ $initIndex+'\"]').text(($initKms*(100-c)/100).toFixed(1));}, 100)}
            else{
                setTimeout(()=>{$('#PorcentajesIntensidad label.perc-ints[data-index=\"'+ $initIndex+'\"]').text(100-c+'%'), 100});
            }
        ...
        "mousemove": function(a) {
            ...
            if($slideType == 1){$('#PorcentajesKilometraje label.perc[data-index=\"'+ $initIndex+'\"]').text(100-c+'%');
            $('#PorcentajesKilometraje label.kms[data-index=\"'+ $initIndex+'\"]').text(($initKms*(100-c)/100).toFixed(1));}else{
                setTimeout(()=>{$('#PorcentajesIntensidad label.perc-ints[data-index=\"'+ $initIndex+'\"]').text(100-c+'%'), 100});
            }
            return this.element.trigger({
                "type": "slide",
