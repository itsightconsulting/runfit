...
! function(a) {
    ...
    b.prototype = {
        ...
        "mousedown": function(b) {
            var inp = b.currentTarget.firstElementChild;
            $initIndex = inp.getAttribute('data-index');
            $initKms = inp.getAttribute('data-kms');
            ...
            var cv = 100 - f;
            if($slideType == 1) {
                const nKms = ($initKms * (cv) / 100);
                $('#PorcentajesKilometraje label.perc[data-index=\"' + $initIndex + '\"]').text(cv+ '%');
                $('#PorcentajesKilometraje label.kms[data-index=\"' + $initIndex + '\"]').text(nKms.toFixed(1));
                $chartTemporada.data.datasets[0].data[$initIndex - 1] = Number(nKms.toFixed(1));
                //Actualizando los datos generales
                /*MacroCiclo.actualizarInformacionKilometrajeTemporada();*/
            } else {
                $('#PorcentajesIntensidad label.perc-ints[data-index=\"'+ $initIndex+'\"]').text(cv+'%')
                $chartTemporada.data.datasets[1].data[$initIndex - 1] = cv;
            }
            $chartTemporada.update();
        ...
        "mousemove": function(a) {
            ...
            if($slideType == 1) {
                $('#PorcentajesKilometraje label.perc[data-index=\"' + $initIndex + '\"]').text(cv+ '%');
                $('#PorcentajesKilometraje label.kms[data-index=\"' + $initIndex + '\"]').text(($initKms * (cv) / 100).toFixed(1));
                $chartTemporada.data.datasets[0].data[$initIndex - 1] = Number(($initKms * (cv) / 100).toFixed(1));
                /*MacroCiclo.actualizarInformacionKilometrajeTemporada();*/
            } else{
                $('#PorcentajesIntensidad label.perc-ints[data-index=\"'+ $initIndex+'\"]').text(cv+'%')
                $chartTemporada.data.datasets[1].data[$initIndex - 1] = cv;
        ...
        "mouseup": function(b) {
            if($slideType == 1) MacroCiclo.actualizarInformacionKilometrajeTemporada()
