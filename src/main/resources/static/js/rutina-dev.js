class Rutina {
    ...
    mostrarSemana(sem, sIndex) {
                        ...
                        `<div class="widget-toolbar borderless" onclick="focoARutina();">
                                                     ...
                                                     <div class="rf-colum-diff">
                                                         ...
                                                         <div class="widget-body">
                                                             ...
                                                             <!-- DIAS DE SEMANA -->
                                                                 <h1 class="text-center">${sIndex != 0 ? `<img class="max-wd" src="${_ctx}img/finish_strong.png"/>` : `<img class="max-wd" src="${_ctx}img/empieza_reto.jpg"/>`}</h1>
                                                                 <!-- END DIAS DE SEMANA -->
                                                             </div>													
                                                         </div>
                                                         <!-- end widget content -->											
        ...
        const arrCirculoFichaClass = ['gr', 'red', 'ylw', 'red', 'gr', 'ylw', 'red'];
        ...
        semana.dias.forEach((v, i) => {
            ...
            rawDias +=
                ...
                <div class="sub-header" role="heading"><span><img class="svg" src="${_ctx}img/iconos-trainers/icon_km.svg"><span class="distancia-total">${parseNumberToDecimal(v.distancia, 2)}</span></span> <span> <img class="svg time" src="${_ctx}img/iconos-trainers/icon_time.svg"><span class="horas-totales">${parseNumberToHours(v.minutos)}</span> </span></div>
    ...
    return {
        ...
        abrirCalendario: (semanasIxs, edicion, mes)=>{
            ...
            calendarSem.forEach((v,i)=>{
                ...
                if(i==0 && v < 7) {
                    for (let z = 0; z < 7-v; z++) {
                        calendarBody+=`<div class="col-md-1 col-sm-1 col-xs-1 col-lg-1  font-md mini txt-color-grayDark text-center">${diaAnteriorAlMes-(6-v)+z}</div>`
                    }
                }
                ...
                if(semanasIxs[0] == 0){
                    for(let x=0; x < v; x++){
                        if(d>= min && d<=max) {
                            calendarBody += `<div class="col-lg-1 col-md-1 col-xs-1 col-sm-1 font-md mini fechas-calendar text-center" data-dia="${d}" data-index="${primSem}">${d++}<i class="fa fa-circle event-calendar"></i></div>`;
                        }else{
                            calendarBody+=`<div class="col-lg-1 col-md-1 col-xs-1 col-sm-1 font-md mini txt-color-grayDark text-center">${d++}</div>`;
                        }
                    }
                    if(d> min && d<=max)
                        primSem++;
                    ...
                    for(let x=0; x < v; x++) {
                        if (d <= max) {
                            calendarBody += `<div class="col-lg-1 col-md-1 col-sm-1 col-xs-1 font-md mini fechas-calendar text-center" data-dia="${d}" data-index="${i}">${d++}<i class="fa fa-circle event-calendar"></i></div>`;
                        } else {
                            calendarBody += `<div class="col-lg-1 col-md-1 col-sm-1 col-xs-1 font-md mini txt-color-grayDark text-center" data-index="${i}">${d++}</div>`;
                        }
                    }
                }
                //Los meses intermedios o full
                    ...
                    for (let x = 0; x < v; x++) {
                        calendarBody += `<div class="col-lg-1 col-md-1 col-sm-1 col-xs-1 font-md mini fechas-calendar text-center" data-dia="${d}" data-index="${i}">
                                            ${d++}
                ...
                if(i==calendarSem.length-1 && v < 7) {
                    for (let z = 0; z < 7-v; z++) {
                        calendarBody+=`<div class="col-lg-1 col-md-1 col-sm-1 col-xs-1 font-md mini txt-color-grayDark text-center">${1+z}</div>`
                    }
                }
                calendarBody += `</div>`;
            })
...
RutinaDelete = (function(){
    return {
        ...
        eliminarEstilosFamiliaFuente: (numSem, diaIndex, eleIndex)=>{
            const estilos = $rutina.semanas[numSem].dias[diaIndex].elementos[eleIndex].estilos;
            estilos.forEach((v,i,k)=>{if(v.id>215 && v.id<226){k.splice(i, 1)}});
        },
    ...
    return {
        agregarOeliminarEstiloToElemento: (estiloId, tipo)=>{
            ...
            if(tipo == 0){
                ...
                RutinaAdd.nuevoEstilo(ixs.numSem, ixs.diaIndex, (posEle = i), estiloId, objEditor.clase, objEditor.tipo);
                $eleGenerico.classList.forEach((v,i,k)=>{v.includes('rf-align')?k.remove(v):''});
                $eleGenerico.classList.add(objEditor.clase);
                guardarEstilosElementoBD(ixs.numSem, ixs.diaIndex, (posEle = i));
            } else if(tipo == 4){
                const ixs = RutinaIx.getIxsForElemento($eleGenerico);
        ...
        obtenerEstilos: (ess)=>{
            let  estTextAlign = "", estElem = "", margen = "";
            for(let i=0; i<ess.length;i++){
                switch(ess[i].tipo) {
                    case 1:
                        ...
                        break;
                    case 2:
                        estTextAlign += ess[i].clase + " ";
                        break;
                    default:
                        margen += ess[i].clase + " "
                }
            }
            return {textAlign: estTextAlign, base: estElem, margen: margen};
        },
        pegarFormato: (input)=>{
            ...
            input.parentElement.parentElement.className = "row";
            let tempEle = RutinaDOMQueries.getElementoByIxs(ixs), i=0;
...
RutinaDOMQueries = (function(){
    return {
        ...
        getElementoByIxs: (ixs)=>{
            return document.querySelector(`#editorContent #rutinaSemana .rf-dia[data-index="${ixs.diaIndex}"] .rf-dia-elemento[data-index="${ixs.eleIndex}"]`);
        },
        getSubElementoByIxs: (ixs)=>{
            return document.querySelector(`#editorContent #rutinaSemana .rf-dia[data-index="${ixs.diaIndex}"] .rf-dia-elemento[data-index="${ixs.eleIndex}"] .rf-sub-elemento[data-index="${ixs.subEleIndex}"]`);
        },
        getPreSubElementoByIxs: (ixs)=>{
            return document.querySelector(`#editorContent #rutinaSemana .rf-dia[data-index="${ixs.diaIndex}"] .rf-dia-elemento[data-index="${ixs.eleIndex}"] .rf-pre-sub-elemento[data-index="${ixs.subEleIndex}"]`);
        },
    ...
    return {
       ...
       //     }
        ...
        agregarMediaToElemento2: (ixs, input)=>{
            ...
            if(dvMediaElements.childElementCount === 0){
                dvMediaElements.innerHTML ="";
            }
                ...
                if(iconoVideo != undefined){//Primer if para actualizar
                    iconoVideo.setAttribute('data-media', $mediaVideo);
                }else{//Para registrar
                    iconoAdd.insertAdjacentHTML('afterend', RutinaElementoHTML.iconoVideo($mediaVideo));
                }
        ...
        agregarInitMediaElemento: (ixs, tipo)=>{
            ...
            if($tipoMedia == TipoElemento.AUDIO){
                ...
                dvIconosMedia.appendChild(htmlStringToElement(`<div class="ong reprod-audio" data-media="${ele.mediaAudio}" rel="tooltip" data-original-title="Audio"></div>`));
            }else{
    ...
    return {
        ...
        agregarMediaSubElemento: (ixs, input, eleIndex, subEleIndex)=>{
            ...
            if($tipoMedia == TipoElemento.AUDIO){
                ...
                //Primer if para actualizar
                if(iconoAudio != undefined){
                    iconoAdd.setAttribute('data-media', $mediaAudio);
                }else{//Para registrar
                   dvMediaElements.appendChild(htmlStringToElement(`<div class="ong reprod-audio" rel="tooltip" data-media="${$mediaAudio}" data-original-title="Audio"></div>`));
...
RutinaSeccion = (function (){
    return {
        newElementoSimple: (diaIndex, tipo, nombre)=>{
            ...
            let elementoHTML = `
                ...
                :   `<i> <img class="svg eliminar-ele-vacio  ele-delete" src="${_ctx}img/iconos-trainers/icon_trash.svg" rel="tooltip" data-placement="bottom" data-original-title="Eliminar" data-dia-index="${diaIndex}" data-index="${ix}"></i>`  }
                    ...
                    `;
             RutinaDOMQueries.getDiaByIx(diaIndex).querySelector('.rt-elem').appendChild(htmlStringToElement(elementoHTML));
            return ix;
        },
        newElementoLista: (diaIndex, tipo, nombre)=>{
            ...
            let subEleHTML = `
                    <div class="panel-group elem rf-dia-elemento" data-index="${ix}" data-type="${tipo}">
                                        <div class="row">
                                         ...
                                         <div class="col-xs-4 dv-tiempo-elem-comp">
                                            <input  type="number" class="item-minutos agregar-tiempo pull-left " maxlength="3" data-index="${ix}" data-dia-index="${diaIndex}" data-placement="top" rel="tooltip" data-original-title="Añadir tiempo en minutos"/>
            ...
            RutinaDOMQueries.getDiaByIx(diaIndex).querySelector('.rt-elem').appendChild(htmlStringToElement(subEleHTML));
            return ix;
        },
        ...
        newSubElemento: (diaIndex, eleIndex, tipo, nombre)=>{
            ...
            let subEleHTML = `
                    <li>
                      <div class="row rf-sub-elemento" data-index="${ix}" data-type="${tipo}">
                      ...
                      <article class="col-xs-12 col-sm-11 col-md-11 col-lg-11 padding-10" data-index="${ix}">         
                        <div class="jarviswidget jarviswidget-color-blueLight margin-bottom-0">
                            <header role="heading" class="header-visor-dia" style="background: #454d54;">
                                <h2><span class="text-center badge bg-color-greenLight font-md txt-color-white padding-5">${objDia.arrIx}</span></h2>
            ...
            return `
                <div class="col-xs-12" style="padding-bottom: 12px;">
                    <div class="input-group" style="width:unset;">
                        ...
                        <div class="input-group-btn">
                            <button onclick="DiaOpc.actualizarObjetivo(this)" class="btn btn-default btn-desp-objetivo" dia-index="${diaIndex}" type="button" style="border-bottom: 1px solid skyblue;border-radius: 0px 20px 20px 0px !important;"><strong><i class="fa fa-check text-success"></i></strong></button>
                        </div>
                    </div>
                </div>`
        }
...
RutinaElementoHTML = (function(){
    return {
        ...
        iconoVideoPlay: (mediaVideo)=>{
            return  `<i><img class="svg reprod-video padding-top-3 rf-media" data-placement="bottom"  rel="tooltip" data-original-title="Reproducir" src="${_ctx}img/iconos-trainers/icon_play.svg" data-media="${mediaVideo}"></i>`
        },
        ...
        iconoNotaT: (nota)=>{
            return `<div class="gr" data-placement="right" data-toggle="popover" data-content="${nota != undefined? nota : ''}" data-trigger="hover"></div>`;
        },
                        ...
                        <div class="row" >
                         <div class="col-xs-8 ds-flex">
                             ...
                             ${ele.nombre.trim() !== '' ?  `<i> <img class="svg insertar-debajo ele-add" src="${_ctx}img/iconos-trainers/icon_add.svg" rel="tooltip" data-placement="bottom" data-original-title="Agregar pares" data-dia-index="${diaIndex}" data-index="${ix}"></i>`
                              ...
                              <div class="notes">
                               ${ele.mediaAudio != undefined && ele.mediaAudio != '' ? `<div class="ong reprod-audio" rel="tooltip" data-media="${ele.mediaAudio}"  data-original-title="Audio"></div>` : ''}
                               ${ele.nota != undefined && ele.nota != '' ? `<div class="gr" data-placement="right" data-toggle="popover" data-content="${ele.nota != undefined? ele.nota : ''}" data-trigger="hover"></div>` : ''}
                                
                              </div>
                              ${ele.mediaVideo != undefined && ele.mediaVideo != ''?RutinaElementoHTML.iconoVideoPlay(ele.mediaVideo):''}
                              <span class="rf-dia-elemento-nombre ${ess.base} ${ess.textAlign}" data-index="${ix}" data-dia-index="${diaIndex}" contenteditable="true" >${ele.nombre}</span>
                         </div>
                         <div class="col-xs-4">
        ...
        elementoCompuestoPaste:(ele, diaIndex)=>{
            ...
            let posPopover = CabeceraOpc.positionPopoverByDiaIndex(diaIndex);
            const val = ele.subElementos.length > 0 ? undefined : 'showInputsInit';//La validacion requiere un null;
            let ix = ++indexGlobal;
            return ` <div class="panel-group elem ${ess.margen} rf-dia-elemento" data-index="${ix}" data-type="${ele.tipo}" data-kms="${ele.distancia}"> 
                ...
                :   `<i> <img class="svg eliminar-ele-vacio  ele-delete" src="${_ctx}img/iconos-trainers/icon_trash.svg" rel="tooltip" data-placement="bottom" data-original-title="Eliminar" data-dia-index="${diaIndex}" data-index="${ix}"></i>`  }
                             ...
                             <div class="notes">
                               ${ele.mediaAudio != undefined && ele.mediaAudio != '' ? `<div class="ong reprod-audio" rel="tooltip" data-original-title="Audio"></div>` : ''}
                               ${ele.nota != undefined && ele.nota != '' ? `<div class="gr" data-placement="right" data-toggle="popover" data-content="${ele.nota != undefined ? ele.nota : ''}" data-trigger="hover"></div>` : ''}
                             </div>
                               ${ele.mediaVideo != undefined && ele.mediaVideo != '' ? RutinaElementoHTML.iconoVideoPlay(ele.mediaVideo) : ''}
                              
                               <span class="rf-dia-elemento-nombre ${ess.base}" data-index="${ix}" data-dia-index="${diaIndex}" contenteditable="true" >${ele.nombre}</span>
                            </div>
                            <div class="col-xs-4 dv-tiempo-elem-comp">
        ...
        subElementoPaste: (sEle, diaIndex, eleIndex)=>{
            ...
            return `<li>
                         <div class="row rf-sub-elemento" data-index="${ix}" data-type="${sEle.tipo}">
                          <div class="col-xs-12 ds-flex">
                          <div class="notes">
                             ${sEle.mediaAudio != undefined && sEle.mediaAudio != '' ? `<div class="ong reprod-audio" rel="tooltip" 
                              data-original-title="Audio" data-media="${sEle.mediaAudio}"></div>` : ''}
                            ${sEle.nota != undefined && sEle.nota != '' ? `<div class="gr" rel="tooltip" data-original-title="Nota"></div>` : ''}
                        </div>
                            ${sEle.mediaVideo != undefined && sEle.mediaVideo != '' ? RutinaElementoHTML.iconoVideoPlay(sEle.mediaVideo) : ''}
                         <span class="pull-left"> 
                         ...
                         <div class="row rf-sub-elemento" data-index="${ix}" data-type="${sEle.tipo}">
                          ...
                          <div class="notes">
                             ${sEle.mediaAudio != undefined && sEle.mediaAudio != '' ? `
                                 <div class="ong reprod-audio" rel="tooltip" data-media="${sEle.mediaAudio}" data-original-title="Audio"></div>` : ''}
                             ${sEle.nota != undefined && sEle.nota != '' ? `<div class="gr" rel="tooltip" data-original-title="Nota"></div>` : ''}
                        </div>
                            ${sEle.mediaVideo != undefined && sEle.mediaVideo != '' ? RutinaElementoHTML.iconoVideoPlay(sEle.mediaVideo) : ''}
                         <span class="pull-left"> 
                      ...
                      <div class="panel-group elem inputs-init ${init == undefined ?  'hidden' : ''}">
                          <div class="panel-group elem" id="dvAddInitElement">
                           ...
                           <div class="row hidden" id="dvInputsInit" >
                            ...
                            <div class="col-xs-6">
                              <input type="text" class="in-ele-dia-2 in-init-ele item-inp" maxlength="121" placeholder="" data-dia-index="${diaIndex}"/>
                            </div>
...
RutinaPS = (function () {
    return {
        opsPopoverElemento: (diaIndex, eleIndex)=>{
            return `
                  <div class="row" id="rMenuEleSubele">
                    <ul>
...
Indicadores = (function(){
    return {
        instanciarIndicador0: ()=>{
            ...
            const ind = intensidad < 61 ? "Baja" : intensidad < 85 ? "Media" : "Alta";
            const contenido = "Intensidad " + ind + " : " + intensidad;
            const btnIndicadorIntensidad = document.querySelector('#IndicadorIntensidad');
            btnIndicadorIntensidad.setAttribute("rel","tooltip");
            btnIndicadorIntensidad.setAttribute("data-placement" , "bottom");
            btnIndicadorIntensidad.setAttribute("data-original-title" , contenido);
        ...
        instanciarPorcentajeAvance: ()=>{
            const btnPorcAvance = document.querySelector('#PorcentajeAvanceSemana');
            const porcActual = Number($rutina.control.avanceSemanas[Number($semActual.textContent)-1]);
            const contPorc= "Cumplimiento: " + porcActual + "%";

            btnPorcAvance.setAttribute("rel","tooltip");
            btnPorcAvance.setAttribute("data-placement" , "bottom");
            btnPorcAvance.setAttribute("data-original-title" , contPorc);

            /*if(porcActual < 80) {
                btnPorcAvance.classList.add('txt-color-redLight');
                btnPorcAvance.classList.remove('txt-color-greenLight');
            }else{
                btnPorcAvance.classList.add('txt-color-greenLight');
                btnPorcAvance.classList.remove('txt-color-redLight');
            }*/
        },
