...
public interface VideoRepository extends JpaRepository<Video, Integer> {
    ...
    @Query("SELECT new Video(E.id, E.nombre, E.rutaWeb, E.rutaReal, E.peso, E.duracion, E.uuid, E.flagActivo, E.subCatVideo.id, E.subCatVideo.nombre) FROM Video E WHERE E.flagActivo = ?1 ORDER BY 1")
    List<Video> findAllByFlagActivoOrderById(Boolean flagActivo);

    List<Video> findAllByNombreContaining(String nombres);

    @Query("SELECT new Video(E.id, E.nombre, E.rutaWeb, E.rutaReal, E.peso, E.duracion, E.uuid, E.flagActivo, E.subCatVideo.id, E.subCatVideo.nombre) FROM Video E WHERE LOWER(E.nombre) LIKE LOWER(CONCAT('%',?1,'%')) ORDER BY 1")
