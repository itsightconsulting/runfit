...
public interface MiniPlantillaRepository extends JpaRepository<MiniPlantilla, Integer> {
    ...
    MiniPlantilla findByTrainerIdAndEspecificacionSubCategoriaId(int trainerId, int especificacionSubCatId);

    @Modifying
    @Query(value = "INSERT INTO mini_plantilla (especificacion_sub_categoria_id, trainer_id) SELECT :id, security_user_id FROM trainer where tipo_trainer_id=2 order by 1", nativeQuery = true)
    ...
    int findPlantillaIdsByTrainerIdAndEspecificacionSubCategoriaId(int id, int esSubCatId);
