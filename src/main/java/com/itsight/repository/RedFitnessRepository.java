...
public interface RedFitnessRepository extends JpaRepository<RedFitness, Integer> {
    ...
    @Query("SELECT NEW com.itsight.domain.dto.RedFitCliDTO(R.id,concat(C.nombres,' ', C.apellidos), R.fechaCreacion ) FROM RedFitness R JOIN R.cliente C WHERE R.trainer.id = ?1 AND R.flagActivo = false AND to_char(R.fechaCreacion, 'YYYY-MM') = ?2")
