...
public interface TrainerRepository extends JpaRepository<Trainer, Integer> {
    ...
    @Query("SELECT T FROM Trainer T INNER JOIN FETCH T.tipoUsuario P JOIN FETCH T.tipoDocumento D  WHERE LOWER(CONCAT(T.apellidoPaterno,' ',T.apellidoMaterno,' ',T.nombres)) LIKE LOWER(CONCAT('%',?1,'%'))")
    ...
    /*@Query(value = "SELECT NEW Trainer(codigoTrainer, nombres, apellidoPaterno, apellidoMaterno) FROM Trainer T WHERE T.tipoUsuario.id = 2")
